...
import {
  ...
  ChevronDown, ChevronUp, MessageCircle, Star, Sparkle,
  LogOut
} from "lucide-react";
...
const iconMap = { Shield, Wrench, Sparkles, Unlock, Calendar, BookOpen, Award, ClipboardList, Video, Landmark, Scale, Eye, FileText, AlertCircle, UserX, CheckSquare, MessageCircle };
...
const initialSubforums = [
  ...
  { id: "events",                categoryId: "main",       title: "პროექტის ღონისძიებები",                  iconName: "Calendar",      topicsCount: 0, postsCount: 0, lastPost: null },
...
const isUserAdmin = (user) =>
  ...
  user?.username?.toLowerCase() === "chavle";
...
function RegisterForm({ onRegister, onLoginClick }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(username)) {
      setError("მხოლოდ ასოები, ციფრები და ქვედა ხაზი");
      return;
    }
        ...
        <div>
          ...
          <div style={{ position:"relative" }}>
            ...
            <button 
              ...
              style={{ position:"absolute", right:"1rem", top:"50%", transform:"translateY(-50%)", color:"var(--grl-muted-foreground)", background:"none", border:"none", cursor:"pointer" }}
            >
        ...
        <div style={{ paddingTop:"1rem", borderTop:"1px solid var(--grl-border)", display:"flex", flexDirection:"column", gap:"0.75rem" }}>
          ...
          <p style={{ textAlign:"center", fontSize:"0.75rem", color:"var(--grl-muted-foreground)" }}>
            ...
            <button 
              ...
              style={{ color:"var(--grl-primary)", fontWeight:700, background:"none", border:"none", cursor:"pointer" }}
            >
              შესვლა
            </button>
          </p>
...
function LoginForm({ onLogin, onRegisterClick }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      localStorage.setItem("grl_current_user", JSON.stringify(userData));
      window.dispatchEvent(new Event("grlAuthChange"));
      onLogin(userData);
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
        <div>
          <label className="grl-label"><User size={14}/> სახელი</label>
          <input 
            type="text" 
            value={username} 
            onChange={e => setUsername(e.target.value)} 
        ...
        <div>
          ...
          <div style={{ position:"relative" }}>
            ...
            <button 
              type="button" 
              onClick={() => setShowPassword(!showPassword)} 
              style={{ position:"absolute", right:"1rem", top:"50%", transform:"translateY(-50%)", color:"var(--grl-muted-foreground)", background:"none", border:"none", cursor:"pointer", padding:"0.25rem" }}
            >
              {showPassword ? <EyeOff size={18}/> : <EyeIcon size={18}/>}
            </button>
          </div>
        ...
        <div style={{ paddingTop:"1rem", borderTop:"1px solid var(--grl-border)", display:"flex", flexDirection:"column", gap:"0.75rem" }}>
          <button 
            ...
            style={{ width:"100%", padding:"0.875rem 1.5rem" }}
          >
            {loading ? <Loader2 size={18} className="grl-animate-spin"/> : <LogIn size={18}/>}
          ...
          <p style={{ textAlign:"center", fontSize:"0.75rem", color:"var(--grl-muted-foreground)" }}>
            ...
            <button 
              ...
              style={{ color:"var(--grl-primary)", fontWeight:700, background:"none", border:"none", cursor:"pointer", fontSize:"0.75rem" }}
            >
              რეგისტრაცია
            </button>
          </p>
  ...
  return (
    <div className="grl-category-block">
      <div className="grl-category-header" onClick={() => setCollapsed(c => !c)}>
        <span className="grl-category-title">{category.title}</span>
...
const syncUsersFromSupabase = async () => {
  try {
    ...
    const supabaseUsers = profiles.map(p => ({
      ...
      email: p.email || '',
      password: existingMap[p.id]?.password || 'temp_password',
      role: p.role || 'player',
      isAdmin: p.is_admin || false,
      avatarBg: p.is_admin 
        ? 'linear-gradient(135deg,#ff2d78,#8b00ff)' 
        : existingMap[p.id]?.avatarBg || 'linear-gradient(135deg,#ec4899,#f43f5e)',
      registeredAt: p.created_at || existingMap[p.id]?.registeredAt || new Date().toISOString(),
    }));
    
    // Keep users that exist only in localStorage (not in Supabase yet)
    const localOnlyUsers = existingUsers.filter(u => 
      !profiles.some(p => p.id === u.id) && 
      !profiles.some(p => p.username === u.username)
    );
    
    const mergedUsers = [...supabaseUsers, ...localOnlyUsers];
    localStorage.setItem('grl_users', JSON.stringify(mergedUsers));
    
    const current = loadCurrentUser();
    if (current) {
      const updated = mergedUsers.find(u => u.id === current.id);
      if (updated) {
        localStorage.setItem('grl_current_user', JSON.stringify(updated));
      }
    }
...
export default function ForumPage() {
  ...
  const [sampServer, setSampServer]   = useState({ online: false, players: 0, maxPlayers: 100 });
  ...
  /* ── სერვერის სტატუსი ── */
  useEffect(() => {
    const getSampStatus = async () => {
      try {
        const res  = await fetch("http://localhost:3001/server-status");
        const data = await res.json();
        setSampServer(data);
      } catch {
        setSampServer({ online: false, players: 0, maxPlayers: 100 });
      }
    };
    getSampStatus();
    const iv = setInterval(getSampStatus, 10000);
  ...
  /* ─── grlAuthChange — sync currentUser-ი users სიიდან ── */
  useEffect(() => {
    const fn = () => {
  ...
  const handleAddReply = () => {
    if (!replyText.trim() || !currentThread?.allowReplies || !currentUser) return;
    const newPost = {
      id: `p-${Date.now()}`,
      author:   currentUser.username,
      avatarBg: currentUser.avatarBg || "linear-gradient(135deg,#ec4899,#f43f5e)",
      role:     currentUser.role || "მოთამაშე",
      roleColor: isAdmin ? "#ff2d78" : "#ec4899",
      content:  replyText,
  ...
  const handleCreateThread = () => {
    ...
    if (!isAdmin && !USER_ALLOWED_SUBFORUMS.includes(currentSubforumId)) return;
    ...
    const newThread = {
      ...
      label: isComplaint ? "განხილვაში" : (isAdmin ? "ადმინი" : "კითხვა"),
      date: new Date().toLocaleDateString("ka-GE"),
      allowReplies: !isComplaint,
      isComplaint, verdict: null, verdictReason: "",
      isAdminPost: isAdmin,
      posts: [{
        id: `p-${Date.now()}`,
        author:   currentUser.username,
      ...
      <main style={{ minHeight:"100vh", padding:"3rem 1.5rem 6rem", position:"relative", zIndex:1 }}>
        <div style={{ maxWidth:"72rem", margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.5rem", flexWrap:"wrap", gap:"0.75rem" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"0.375rem", fontSize:"0.75rem", color:"var(--grl-muted-foreground)", fontWeight:600, flexWrap:"wrap" }}>
              <button onClick={navHome} style={{ background:"none", border:"none", cursor:"pointer", color:"var(--grl-muted-foreground)", fontWeight:600, fontSize:"0.75rem", padding:0 }}
              ...
              {viewState==="subforum" && currentSubforum && (
                <div className="grl-animate-fadein" style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"0.75rem" }}>
                    ...
                    {isAdmin || USER_ALLOWED_SUBFORUMS.includes(currentSubforumId) ? (
                      <button onClick={() => { if (!currentUser) { setShowAuth(true); return; } if (!isAdmin && !USER_ALLOWED_SUBFORUMS.includes(currentSubforumId)) return; setIsModalOpen(true); }} className="grl-btn grl-btn-primary grl-btn-sm">
                        <Plus size={14}/> ახალი თემა
                      </button>
                    ) : (
                  ...
                  <div className="grl-category-block">
                    ...
                    <div className="grl-thread-header">
                      <span>თემა / ავტორი</span>
                      <span style={{ textAlign:"center" }}>პასუხი</span>
                        ...
                        <div key={t.id} className="grl-thread-row" style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
                            <div style={{ fontSize:"0.65rem", color:"var(--grl-muted-foreground)", textAlign:"right" }}>
                              <div>{t.posts?.length || 0} პოსტი</div>
                            </div>
                            {isAdmin && (
                              <button
                                onClick={() => handleDeleteThread(t.id)}
                                className="grl-btn-danger"
              ...
              {viewState==="thread" && currentThread && (
                <div className="grl-animate-fadein" style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
                  <button onClick={goBack}
                    ...
                    onMouseEnter={e=>e.currentTarget.style.color="var(--grl-primary)"}
                    onMouseLeave={e=>e.currentTarget.style.color="var(--grl-muted-foreground)"}>
                    <ArrowLeft size={14}/> თემებში დაბრუნება
                  </button>
                    ...
                    <>
                      <div style={{ display:"flex", flexWrap:"wrap", gap:"0.375rem", marginBottom:"0.875rem" }}>
                        {onlineMembers.map((m,i) => {
                          ...
                          const isAdminM  = m.toLowerCase() === "chavle" || isUserAdmin(currentUser);
                          return (
        ...
        {isModalOpen && (
          <div className="grl-modal-overlay" onClick={e=>e.target===e.currentTarget&&setIsModalOpen(false)}>
            <div className="grl-card grl-card-3xl grl-animate-fadein" style={{ width:"100%", maxWidth:"34rem", padding:"1.75rem", position:"relative", boxShadow:"0 25px 60px oklch(0 0 0/0.5)" }}>
              <h3 style={{ fontSize:"1rem", color:"oklch(0.97 0.005 250)", textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:"1.25rem", paddingBottom:"0.75rem", borderBottom:"1px solid var(--grl-border)", fontFamily:"var(--grl-font-display)" }}>
                {COMPLAINT_SUBFORUMS.includes(currentSubforumId) ? "საჩივრის გაგზავნა" : "ახალი თემის შექმნა"} — {currentSubforum?.title}
              </h3>
              <div style={{ display:"flex", flexDirection:"column", gap:"1.125rem" }}>
                <div>
                  <label className="grl-label">სათაური</label>
                  <input type="text" value={newThreadTitle} onChange={e=>setNewThreadTitle(e.target.value)}
                    placeholder={COMPLAINT_SUBFORUMS.includes(currentSubforumId) ? "მაგ: საჩივარი მოთამაშეზე | 0045" : "მაგ: ახალი განახლება"}
                    className="grl-input"/>
                </div>
                <div>
                  <label className="grl-label">შინაარსი</label>
                  <textarea rows={5} value={newThreadContent} onChange={e=>setNewThreadContent(e.target.value)} placeholder="დეტალურად აღწერეთ…" className="grl-input"/>
                </div>
                <div style={{ display:"flex", justifyContent:"flex-end", gap:"0.75rem", paddingTop:"0.875rem", borderTop:"1px solid var(--grl-border)" }}>
                  ...
                  <button type="button" onClick={handleCreateThread} className="grl-btn grl-btn-primary grl-btn-sm">
                    {COMPLAINT_SUBFORUMS.includes(currentSubforumId) ? "საჩივრის გაგზავნა" : "გამოქვეყნება"}
                  </button>
                </div>
              </div>
            </div>
